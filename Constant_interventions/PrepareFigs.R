...
popsizes <- read.table('final_frequencies_with_interventions.txt', row.names=NULL,header=T)
...
# image.plot(matrix(freqs[get_subset(c('r12'=1,'r21'=0.1,'p12'=1.1,'p22'=1.1, 'MD'=MD[i],'Ig'=Ig[j],'q2'=q2[k],'is2'=is2[l]))], nrow=15),zlim=c(0,1))
		...
		for (p1 in p12)
			for (p2 in p22)
				for (quar2 in q2)
					for (iso2 in is2)
						{
						pdf(file=paste('ResultsFinalValues/results_prev',k,'.pdf',sep=''))
						par(mfrow=c(2,2))
						draw(r1, r2, p1, p2, quar2, iso2)
						dev.off()
						k <- k + 1
					}

library(animation)
animation_options <- list(interval=0.25, loop=1, verbose=FALSE)
...
for (r1 in r12)
	for (r2 in r21)
			...
			movie.name=paste('ani',k,'.gif',sep='')
			)
			}

for (i in 1:k)
	{
	system(paste('ffmpeg -i ani',i,'.gif -crf 12 -b:v 500K ani',i,'.webm',sep=''))
	}


setwd('ResultsFinalValues')
system('pdfunite $(ls -v results_prev*.pdf) ../prevalence_constant_interventions.pdf')
setwd("..")
...
draw <- function(r1,r2,p12,p22,q2,is2)
{
  for (i in 1:2)
    for (j in 1:2)
    {
      image(x=log(q1,10),y=log(is1,10),z=matrix(s2[get_subset(c('r12'=r1,'r21'=r2,'p12'=p12,'p22'=p22, 'MD'=MD[i],'Ig'=Ig[j],'q2'=q2,'is2'=is2))], nrow=15,byrow=TRUE),xlab='Log 10 isolation effort',ylab='Log 10 quarantine effort',col=tim.colors(64),main=paste('med help=',MD[i],'Ignored=',Ig[j],'h1 risk=',r1,'mixing=',r2,'\nphen h1=',p12,'phen h2=',p22,'q help=',q2,'is help=',is2),cex.main=0.8,zlim=range(s2,na.rm=T))
    }
}
...
drawF <- function(r1,r2,p12,p22,q2,is2)
{
  for (i in 1:2)
    for (j in 1:2)
    {
      image(x=log(q1,10),y=log(is1,10),z=matrix(freqs[get_subset(c('r12'=r1,'r21'=r2,'p12'=p12,'p22'=p22, 'MD'=MD[i],'Ig'=Ig[j],'q2'=q2,'is2'=is2))], nrow=15),xlab='Log 10 isolation effort',ylab='Log 10 quarantine effort',col=tim.colors(64),main=paste('med help=',MD[i],'Ignored=',Ig[j],'h1 risk=',r1,'mixing=',r2,'\nphen h1=',p12,'phen h2=',p22,'q help=',q2,'is help=',is2),cex.main=0.8,zlim=range(freqs,na.rm=T))
    }
}
    ...
    for (p1 in p12)
      for (p2 in p22)
        for (quar2 in q2)
          for (iso2 in is2)
          {
            pdf(file=paste('ResultsFinalValues/freq_results',k,'.pdf',sep=''))
            par(mfrow=c(2,2))
            drawF(r1, r2, p1, p2, quar2, iso2)
            dev.off()
            k <- k + 1
          }

setwd('ResultsFinalValues')
system('pdfunite $(ls -v freq_results*.pdf) ../constant_interventions_freqs.pdf')
